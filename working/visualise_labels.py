...
"""
Visualisation tools for plotting SARFish imagery and associated labels and predictions.

Sourced from: SARFish challenge repository (https://github.com/DIUx-xView/SARFish)
...
def scale_sentinel_1_image(
    ...
    ) -> np.ndarray: # PJM: return type is a processed NumPy array
    """
    ...
    output = np.zeros_like(data, dtype = np.float32) # PJM: initialize the output array (same shape as data)
    valid_data = convert_sentinel_1_image_to_tensor(data[~nodata_mask], product_type) # PJM: data[~nodata_mask] filters out invalid pixels, ensures only valid values passed
    
    if product_type == "SLC":
        # PJM: Logarithmic Scaling for SLC image
        valid_data += 0.1 # PJM: epsilon to avoid log10(0)
        valid_data = np.abs(valid_data)**2 # PJM: Compute squared magnitude

        scaled_valid_data = 10 * torch.log10(valid_data) # PJM: Convert to decibels
        output[~nodata_mask] = scaled_valid_data.numpy() # PJM: only valid pixels filled with scaled values in output
        return output

    # PJM: GRD is scaled directly (without squaring)
