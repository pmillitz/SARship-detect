...
with shape (H, W, 3) and converts the data to uint8 PNG format where the first
channel of the third axis is the extracted amplitude which is clipped, db-scaled
and normalised to [0, 1] in that order, the second channel is the extracted phase
normalised to [0, 1] and the third channel is padded with zeros. 

usage: 
    # Process single file
    python complex_scale_and_norm_png.py --input-dir input_array.npy --output-dir /results/processed/
    
    # Process all .npy files in a directory  
    python complex_scale_and_norm_png.py --input-dir images/ --output-dir /results/images/
    
    # Directory processing with amplitude clipping 
    python complex_scale_and_norm_png.py --input-dir images_hvrt_bal/ --output-dir /results/images_hvrt_bal/ --amp-clip-params 1.00 71.51
"""
...
def process_complex_data(slc_data, epsilon=1e-6, nan_strategy='skip', verbose=False, amp_clip_params=None):
    ...
    phase, applies scaling and normalisation, then outputs RGB uint8 format.
    ...
    # Simple phase processing - no clipping needed
    phase = np.angle(slc_data)  # Returns (-π, π]
    phase = (phase + np.pi) / (2 * np.pi)  # Normalize to [0, 1]
    
    # Assign to RGB channels
    rgb_data[:, :, 0] = amplitude  # Red channel
    rgb_data[:, :, 1] = phase      # Green channel
    rgb_data[:, :, 2] = 0          # Blue channel (zeros)
...
def main():
    parser = argparse.ArgumentParser(
        description="Process complex-valued SAR data into RGB PNG format for YOLO training",
...
Output Format:
  ...
  - Red channel: Clipped amplitude → dB scale → normalized
  - Green channel: Phase normalized from (-π, π] to [0, 1]
  - Blue channel: Zeros (for YOLO compatibility)
...
Amplitude Clipping:
  ...
  - Use this for consistent normalization across multiple SAR images
    ...
    # Batch processing mode
        ...
        if args.verbose:
            print("=" * 60)
            print("SAR Batch Processing Mode")
            print("=" * 60)
